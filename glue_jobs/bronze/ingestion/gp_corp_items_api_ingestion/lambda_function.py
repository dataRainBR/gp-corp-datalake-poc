...
RECORDS_PER_FILE = 1000  # Máximo de registros por arquivo JSON


def save_to_s3(data, entity, load_type="full"):
    """
    Salva dados no S3 em formato JSON, particionado por data.
    Divide em arquivos de no máximo 1000 registros para evitar JSONs grandes.
    """
    if not data:
    ...
    now = datetime.utcnow()
    total_parts = (len(data) + RECORDS_PER_FILE - 1) // RECORDS_PER_FILE

    for i in range(0, len(data), RECORDS_PER_FILE):
        chunk = data[i:i + RECORDS_PER_FILE]
        part_num = i // RECORDS_PER_FILE + 1

        # Se só tem 1 parte, não adiciona sufixo _partN
        if total_parts == 1:
            filename = f"{entity}_{now.strftime('%Y%m%d_%H%M%S')}.json"
        else:
            filename = f"{entity}_{now.strftime('%Y%m%d_%H%M%S')}_part{part_num}.json"
        ...
        json_content = json.dumps(chunk, default=str, ensure_ascii=False)
        ...
        if total_parts > 1:
            print(f"  ✓ {entity} part {part_num}/{total_parts}: {len(chunk)} registros → s3://{S3_BUCKET}/{s3_key}")

    print(f"  ✓ {entity}: {len(data)} registros salvos em {total_parts} arquivo(s)")
